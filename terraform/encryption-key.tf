...
resource "aws_kms_key" "active_suspensions_details_dynamodb_kms_key" {
  description = "Custom KMS Key to enable server side encryption for active-suspensions DB"
  policy      = data.aws_iam_policy_document.kms_key_policy_doc.json
  enable_key_rotation = true

  tags = {
    Name        = "${var.environment}-${var.component_name}-active-suspensions-details-dynamodb-kms-key"
    CreatedBy   = var.repo_name
    Environment = var.environment
  }
}

resource "aws_kms_alias" "active_suspensions_details_dynamodb_encryption" {
  name          = "alias/${var.component_name}-active-suspensions-details-dynamodb-encryption-kms-key"
  target_key_id = aws_kms_key.active_suspensions_details_dynamodb_kms_key.id
}
